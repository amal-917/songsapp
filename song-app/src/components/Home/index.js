...
class Home extends Component {
    ...
    searchSong = (event) => {
        ...
        this.setState({userInput:userInputOne,song: Songs.filter(each => each.name.toLowerCase().includes(userInputOne.toLowerCase()))})
    }
    ...
    <div className="main-background-container container">
        <div className="top-container">
            ...
            <div className="filters-container">
                <button type="button" onClick = {this.getAll} className = {`filters ${isTrue}`} >
                    <p>All</p>
                </button>
                <button type="button" onClick = {this.getTrending} className={`filters ${isTrue}`} >
                    <p>Trending</p>
                </button>
                <button type="button" onClick ={this.newRelease} className={`filters ${isTrue}`}>
                    <p>New Release</p>
                </button>
            </div>
            <div className="crousal">
            <Carousel slides={Songs.map((each, index)=> (
                <>
                <div style={{backgroundImage: `url(${each.cover})`}} className="song-carousal-container" key ={uuid()}>
                <p className="name">{each.name}</p>
                <p>{each.artist}</p>
                </div>
                </>
            ))} autoplay={false} className="arrows"/>
            </div>
            <div className="thumbs">
                {
                    song.map((each,index) => (
                        <div className="song-thumb-container" key ={uuid()}>
                            <Link to= {{pathname:`/${each.name}` , state : {one: each},}} className="link">
